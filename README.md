...
- У каждой колоды три кнопки:
	...
	- "Удалить" - удаление с подтверждением: "Удалить колоду и все её карточки? Это действие нельзя отменить". Каскадное удаление
...
- У каждой карточки две кнопки:
	...
	- "Удалить" - удаление с подтверждением: "Удалить карточку? Это действие нельзя отменить"
### 6.3 Окно импорта
Элементы:
- Выпадающий список для выбора колоды
- Выбор кодировки файла: UTF-8 или Windows-1251
- Кнопка выбора CSV-файла
- Кнопка "Импортировать"
- По завершении всплывающее окно: "Добавлено карточек: X, пропущено: Y"
- При ошибке кодировки - сообщение с предложением сменить кодировку
### 6.4 Окно повторения
Поведение:
- Пользователь выбирает колоду
- Если карточек для повторения нет - сообщение "Все карточки повторены на сегодня" и возврат в меню
...
- В окне импорта пользователь выбирает кодировку: UTF-8 (по умолчанию) или Windows-1251
- Файл открывается с указанной кодировкой: open(path, encoding='utf-8') или open(path, encoding='cp1251')
...
# 10. Отложенные функции (после MVP)
